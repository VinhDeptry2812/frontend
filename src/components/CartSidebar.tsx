          ...
          >
                    ...
                    <div key={item.id} className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <p className="font-serif font-bold text-sm">${(item.price * item.quantity).toLocaleString()}</p>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            {items.length > 0 && (
              <div className="p-6 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800">
                ...
                <button className="w-full bg-primary text-white py-4 rounded-full font-bold flex items-center justify-center gap-2 hover:bg-blue-700 transition-all shadow-lg shadow-primary/20">
                  Proceed to Secure Checkout
                  <ArrowRight size={18} />
                </button>
                <p className="text-[10px] text-center text-slate-400 mt-4 uppercase tracking-widest">
                  Free white-glove delivery on orders over $5,000
                </p>
              </div>
            )}
          </motion.div>
