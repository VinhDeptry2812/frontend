...
export const AdminProducts: React.FC = () => {
  ...
  const fetchProducts = async (cursorToFetch: string = cursorHistory[currentPage] || '', isReset: boolean = false) => {
    try {
      ...
      setProducts(data); // Optimized: Don't fetch variants for list items to save time/resources
  ...
  const openEditModal = (prd: ProductData) => {
    ...
    setFormData({
      ...
      sku: prd.sku || '',
      category_id: prd.category_id?.toString() || prd.category?.id?.toString() || '',
                        ...
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-lg bg-slate-100 overflow-hidden flex-shrink-0 border border-slate-200">
                              {prd.image_url ? (
                                <img src={prd.image_url} alt={prd.name} className="w-full h-full object-cover" />
                              ) : (
                            ...
                            <div className="flex flex-col">
                              <span className="font-semibold text-slate-900 line-clamp-1">{prd.name}</span>
                              <div className="flex gap-1.5 mt-0.5">
                                {prd.is_featured && <span className="text-[9px] font-bold bg-amber-100 text-amber-700 px-1 py-0.5 rounded uppercase tracking-wider">🌟 Nổi bật</span>}
                                {!prd.is_active && <span className="text-[9px] font-bold bg-red-100 text-red-700 px-1 py-0.5 rounded uppercase tracking-wider">Ẩn</span>}
                              </div>
                            </div>
                          </div>
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl">
            ...
            {modalMode === 'view' && currentProduct ? (
                          ...
                          {currentProduct.variants.map((v: any, idx: number) => (
                            <tr key={v.id || idx} className="hover:bg-slate-100 transition-colors">
                              ...
                              <td className="px-4 py-3 font-bold text-slate-900">
                                {v.sale_price ? new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(v.sale_price) : 'Dùng giá gốc'}
                              </td>
                              <td className="px-4 py-3 text-right">
                                <span className={`font-bold ${v.stock_quantity <= 5 ? 'text-orange-500' : 'text-slate-900'}`}>
                                  {v.stock_quantity || 0}
                                </span>
                              </td>
                            </tr>
              ...
              <form id="productForm" onSubmit={handleFormSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-slate-600 mb-1.5">Thương hiệu</label>
                        <input type="text" value={formData.brand} onChange={e => setFormData({ ...formData, brand: e.target.value })} className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 focus:border-primary focus:ring-1 focus:ring-primary shadow-sm" placeholder="VD: Luxe Interiors" />
                    </div>
                    
                    <div>
                        <label className="block text-sm font-semibold text-slate-600 mb-1.5">Chất liệu</label>
                        <input type="text" value={formData.material} onChange={e => setFormData({ ...formData, material: e.target.value })} className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 focus:border-primary focus:ring-1 focus:ring-primary shadow-sm" placeholder="VD: Gỗ sồi, Da bò" />
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" value={formData.sale_price} onChange={e => setFormData({ ...formData, sale_price: e.target.value })} className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 focus:border-primary focus:ring-1 focus:ring-primary shadow-sm" placeholder="Để trống nếu không giảm" />
                    </div>

                    <div>
                        <label className="block text-sm font-semibold text-slate-600 mb-1.5">SKU (Mã sản phẩm)</label>
                        <input type="text" value={formData.sku} onChange={e => setFormData({ ...formData, sku: e.target.value })} className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 focus:border-primary focus:ring-1 focus:ring-primary shadow-sm" placeholder="VD: SFA-001" />
                    </div>
                    ...
                    <div className="col-span-2 flex items-center gap-6 py-2">
                        <label className="flex items-center gap-2 cursor-pointer group">
                            <input type="checkbox" checked={formData.is_active} onChange={e => setFormData({ ...formData, is_active: e.target.checked })} className="w-4 h-4 text-primary rounded border-slate-300 focus:ring-primary" />
                            <span className="text-sm font-bold text-slate-700 group-hover:text-primary transition-colors">Đang kinh doanh</span>
                        </label>
                        <label className="flex items-center gap-2 cursor-pointer group">
                            <input type="checkbox" checked={formData.is_featured} onChange={e => setFormData({ ...formData, is_featured: e.target.checked })} className="w-4 h-4 text-primary rounded border-slate-300 focus:ring-primary" />
                            <span className="text-sm font-bold text-slate-700 group-hover:text-primary transition-colors">Nổi bật 🌟</span>
                        </label>
                        <label className="flex items-center gap-2 cursor-pointer group">
                            <input type="checkbox" checked={formData.is_new} onChange={e => setFormData({ ...formData, is_new: e.target.checked })} className="w-4 h-4 text-primary rounded border-slate-300 focus:ring-primary" />
                            <span className="text-sm font-bold text-slate-700 group-hover:text-primary transition-colors">Hàng mới về ✨</span>
                        </label>
                    </div>
                </div>
                ...
                <div>
                  ...
                  <textarea rows={3} value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 focus:border-primary focus:ring-1 focus:ring-primary shadow-sm placeholder:text-slate-300" placeholder="Mô tả tóm tắt về sản phẩm..." />
                </div>
